...
2. Ensure the SimplyBudgetWeb is running at the configured base URL
...
```bash
dotnet run --project SimplyBudgetWeb.AppHost
```
...
- name: Install Playwright Browsers
  run: pwsh SimplyBudgetWeb.UITests/bin/Debug/net10.0/playwright.ps1 install --with-deps

- name: Run UI Tests
  env:
    TEST_BASE_URL: https://staging.example.com
  run: dotnet test SimplyBudgetWeb.UITests/SimplyBudgetWeb.UITests.csproj
```
...
- task: PowerShell@2
  ...
  inputs:
    targetType: 'inline'
    script: |
      pwsh SimplyBudgetWeb.UITests/bin/Debug/net10.0/playwright.ps1 install --with-deps

- task: DotNetCoreCLI@2
  displayName: 'Run UI Tests'
  inputs:
    command: 'test'
    projects: 'SimplyBudgetWeb.UITests/SimplyBudgetWeb.UITests.csproj'
  env:
