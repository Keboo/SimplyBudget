...
.DESCRIPTION
    ...
    7. Generates Terraform variable files for deployment
...
Write-Host "    4. Generate Terraform variable files" -ForegroundColor White
...
# Remove legacy file from older infra pattern if it exists
$spTfPath = Join-Path (Join-Path $infraDir "prod") "service_principals.tf"
if (Test-Path $spTfPath) {
    Remove-Item -Path $spTfPath -Force
    Write-Host "  Removed legacy file $spTfPath" -ForegroundColor Green
}
